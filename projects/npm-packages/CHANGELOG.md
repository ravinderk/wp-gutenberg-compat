...
- Run `build` during npm packing/publishing via `prepack` so published tarballs include `dist/` and the `wp-gutenberg-compat` CLI binary resolves correctly when used with `npx`.

## [0.5.0] - 2026-03-25

### Added

- `info` command output now includes detected plugin/theme name and project type.

### Changed

- Migrated `projects/npm-packages` source code from JavaScript to TypeScript, including CLI commands, analysis utilities, and generation scripts.

## [0.4.0] - 2026-03-24

### Added

- `open` subcommand to open a `@wordpress/*` package page on npmjs.com.
- README documentation for the `open` command.

### Changed

- Converted project to npm workspaces monorepo layout (`projects/npm-packages/`).
- Split `generate.js` into focused `lib/` modules (`utils`, `github`, `gutenberg`, `wp-gb-map`).
- Use additive merge for upstream WP-Gutenberg map entries in data generation.

### Fixed

- Added fallback in `resolveRangeInCompatData` when a semver range's lower bound exceeds the highest tracked compat-data version, preventing false "all compatible" results.
...
[0.5.1]: https://github.com/ravinderk/wp-gutenberg-compat/compare/npm-packages/v0.5.0...npm-packages/v0.5.1
[0.5.0]: https://github.com/ravinderk/wp-gutenberg-compat/compare/npm-packages/v0.4.0...npm-packages/v0.5.0
[0.4.0]: https://github.com/ravinderk/wp-gutenberg-compat/compare/0.3.0...0.4.0
